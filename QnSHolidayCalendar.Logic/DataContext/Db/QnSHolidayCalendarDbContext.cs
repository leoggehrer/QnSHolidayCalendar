...
using QnSHolidayCalendar.Logic.Entities.Persistence.App;

namespace QnSHolidayCalendar.Logic.DataContext.Db
{
    internal partial class QnSHolidayCalendarDbContext
    {
        static QnSHolidayCalendarDbContext()
        {
            ClassConstructing();
...
#endif
        private static string ConnectionString { get; set; } = "Data Source=(localdb)\\MSSQLLocalDb;Database=QnSHolidayCalendarDb;Integrated Security=True;";

        public QnSHolidayCalendarDbContext()
        {
            Constructing();
            Constructed();
        }
        ...
        partial void ConfigureEntityType(EntityTypeBuilder<CalendarEntry> entityTypeBuilder)
        {
            entityTypeBuilder
                .Property(p => p.Description)
                .IsRequired()
